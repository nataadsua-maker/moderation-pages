...
# быстрые 500 успевают отработать все попытки, а зависание обрывается сразу.
#
# 90 -> 50 (вечер 23.09): при 90с на кадр помещались ДВА зависания по 45с, и
# второе — чистая трата: на перепрогоне REQ-260923-203 (48 кадров) шаг выел все
# 25 минут и был убит. Замер: успешный ответ приходит за 14с (медиана), самый
# медленный живой — 37с, а зависший висит 45с+. Различить их таймаутом нельзя
# (резать до 20с нельзя — потеряем 3 живых ответа из 10), поэтому бросаем после
# ПЕРВОГО зависания. Быстрые 500 при этом по-прежнему дожимаются всеми попытками.
VISION_BUDGET = float(os.environ.get("NIM_VISION_BUDGET", "50"))
